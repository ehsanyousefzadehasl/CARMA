...
def plot_subfigures_for_one_gpu(data1_dir, data2_dir, gpu_id, metrics, smooth):
    ...
    for idx, metric in enumerate(metrics):
        ...
        axs[idx].plot(d1, label=label1, linestyle='--', color='LightCoral', linewidth=1.5)
        axs[idx].plot(d2, label=label2, linestyle='-.', color='DarkRed', linewidth=2)
        ...
        axs[idx].legend(loc='lower right', frameon=False)
        axs[idx].grid(True, linestyle='--', linewidth=0.5, color='0.85')
        axs[idx].set_xlabel("Time (3-second intervals)")

    axs[-1].set_xlabel("Time (3-second intervals)")
    plt.tight_layout()
    filename = f"GPU{gpu_id}_all_metrics_comparison-poster{'_smoothed' if smooth else ''}.pdf"
